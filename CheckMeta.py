...
BOT_TOKEN = "7798097932:AAFpm3H-iwP57zbJ8yQ_72RJBfGJNttew6o"  # Telegram Bot Token
...
def parse_time(time_str):
    units = {"s": "seconds", "m": "minutes", "h": "hours", "d": "days", "y": "year" }
    try:
...
async def approve(event):
    ...
    try:
        ...
        if access_time:
            ...
            await event.reply("❌ Invalid time format. Use s, m, h, d, or 1 (e.g., 6s, 3m, 9h, 5d , 1y ).")
...
async def start(event):
    ...
    if user_id in approved_users and datetime.now() < approved_users[user_id]:
        ...
        await event.reply("❌ You are not approved to use this command.\n**DM @Mepluger to get approval.**",
                          buttons=[Button.url('𝗝𝗼𝗶𝗻 𝗖𝗵𝗮𝗻𝗻𝗲𝗹✅', 'https://t.me/unknpluger')])
...
async def user_info(event):
    ...
    try:
        ...
        result = f"**Instagram User Info:** **[~ || Mepluger ||]**(tg://openmessage?user_id=6066188248)\n\n"
